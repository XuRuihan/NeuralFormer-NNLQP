...
class Trainer(object):
    def __init__(self, args):
        ...
        if self.args.dataset == "nnlqp":
            if self.args.train_test_stage:
                ...
                (
                    self.train_loader,
                    self.test_loader,
                ) = self.init_unseen_structure_dataset()
        else:
            self.init_accuracy_dataset()

        self.device = torch.device(
            "cuda" if self.args.gpu >= 0 and torch.cuda.is_available() else "cpu"
        )
        ...
        if not self.args.only_test:
            ...
            self.scheduler = get_linear_schedule_with_warmup(
                self.optimizer,
                num_warmup_steps=self.args.warmup_rate * total_iters,
    ...
    def init_accuracy_dataset(self):
        if self.args.only_test:
            test_set = AccuracyDataset(
                self.args.dataset, "test", self.args.data_root, self.args.train_num
            )
            test_sampler = FixedLengthBatchSampler(
                test_set, self.args.batch_size, include_partial=True
            )
            self.test_loader = TorchDataLoader(
                test_set, shuffle=(test_sampler is None), batch_sampler=test_sampler
            )
            self.logger.info("Test data = {}".format(len(test_set)))

        else:
            train_set = AccuracyDataset(
                self.args.dataset,
                "train",
                self.args.data_root,
                self.args.train_num,
                self.args.lambda_cons > 0,
                self.args.multires_x,
                self.args.multires_p,
                self.args.embed_type,
                self.args.optype,
            )
            test_set = AccuracyDataset(
                self.args.dataset, "val", self.args.data_root, self.args.train_num
            )  # Valition dataset
            self.logger.info(
                "Train data = {}, Val data = {}".format(len(train_set), len(test_set))
            )
    ...
    def init_model(self):
        ...
        if self.args.resume:
            ...
            self.logger.info(
                "loaded checkpoint: {} (epoch {}  best acc{:.2f}  best tau{:.2f})".format(
                    self.args.resume, start_epoch, best_acc, best_tau
                )
            )

            self.logger.info(
                "Loading optimizer and scheduler: {}".format(self.args.resume)
            )
            self.optimizer.load_state_dict(ckpt["optimizer"])
            self.scheduler.load_state_dict(ckpt["scheduler"])
            self.logger.info(
                "Loaded optimizer and scheduler: {}".format(self.args.resume)
            )
    ...
    def train_epoch(self, epoch):
        ...
        for iteration, batch in enumerate(self.train_loader):
            ...
            if self.args.lambda_cons > 0:
                source_pre, auged_pre = torch.split(
                    pred_cost, pred_cost.shape[0] // 2, dim=0
                )
            ...
            if iteration % self.args.print_freq == 0 or iteration + 1 == num_iter:
                ...
                if self.args.lambda_sr > 0 and self.args.lambda_cons > 0:
                    self.logger.info(
                        "Loss_SR:{:.5f} Loss_consistency:{:.5f}".format(
                            loss_sr.data, loss_cons.data
                        )
                    )
    ...
    def test(self):
        ...
        if self.args.only_test:
            self.logger.info(
                " Average Latency : {:.8f} ms".format(infer_time / num_iter * 1000)
            )
    ...
    def train(self):
        ...
        self.logger.info(
            "Train over, best acc = {:.5f}, err = {}, tau = {}".format(
