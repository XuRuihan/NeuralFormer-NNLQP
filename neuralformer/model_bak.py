...
class GroupLinear(nn.Module):
    ...
    def __init__(
        ...
        if in_features % n_groups != 0:
            err_msg = "Input dimensions ({}) must be divisible by n_groups ({})".format(
...
class Net(nn.Module):
    def __init__(
        ...
        for j in range(n_attned_gnn):
            ...
            self.layer_scale.append(
                nn.Parameter(init_values * torch.ones((gnn_hidden)), requires_grad=True)
            )
    ...
    def forward(self, data1, data2, n_edges):
        ...
        if self.dataset == "nnlqp":
            ...
            for i in range(idx.size(0)):
                ...
                for gnn, dropout, relu, ffn, gamma in zip(
                    self.gnn_layers,
                    self.gnn_drops,
                    self.gnn_relus,
                    self.FFN_layers,
