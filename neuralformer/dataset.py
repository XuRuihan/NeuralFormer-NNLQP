...
from .data_process.data_and_encoding_generate import (
    ac_aug_generate,
    padding_for_batch,
    tokenizer,
)
...
def get_torch_data(onnx_file, batch_size, cost_time, embed_type):
    adjacent, node_features, static_features, topo_features = extract_graph_feature(
        onnx_file, batch_size, embed_type
    )
    ...
    def load(self):
        ...
        for key in keys:
            ...
            if self.use_aug and self.part == "train":
                auged_opss, auged_adjs = ac_aug_generate(
                    datas[key]["ops"], datas[key]["adj"], datas[key]["num_vertices"]
                )
                if len(auged_adjs) > 0:
                    total += 1
                    auged_code = tokenizer(
                        auged_opss[0],
                        auged_adjs[0],
                        self.dx,
                        self.dp,
                    ...
                    auged_code, auged_adj = (
                        padding_for_batch(auged_code, auged_adjs[0])
    ...
    def preprocess(self, data):
        if self.dataset == "nasbench101":
            ...
            V_label = torch.tensor(
                [data["validation_accuracy_avg"]], dtype=torch.float32
            )
...
class GraphLatencyDataset(Dataset):
            ...
            for line in f.readlines():
                ...
                if os.path.exists(onnx_file):
                    data_file = os.path.join(
                        self.processed_dir, "{}_{}_data.pt".format(speed_id, plt_id)
                    )
                    sf_file = os.path.join(
                        self.processed_dir, "{}_{}_sf.pt".format(speed_id, plt_id)
                    )
    ...
    def get(self, idx):
        ...
        if self.selfpos > 0:
            ...
            sp_coding = [
                fn(np.array([int(i)], dtype="float32")) for i in range(data.x.size(0))
            ]
