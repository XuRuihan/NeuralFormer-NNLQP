...
class Scale(nn.Module):
    def __init__(self, dim, init_values=1.0):
        super().__init__()
        self.scale = nn.Parameter(init_values * torch.ones(dim), requires_grad=True)
...
class GNN_LinearAttn(nn.Module):
    ...
    def forward(self, x, A):
        ...
        # out (B, N, D2)
        return self.proj(F.dropout(F.relu(out), p=0.05)) + x
        # return F.dropout(F.relu(out), p=0.05)
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Optional[Tensor] = None) -> Tensor:
        ...
        # return self.norm(self.layer_scale(self.drop_path(x_)) + x)
        x_ = self.norm(x)
        x_ = self.feed_forward(x_, adj)
        return self.drop_path(x_) + self.res_scale(x)
...
class PreReduction(nn.Module):
    ...
    def forward(self, x):
        # return F.layer_norm(x, [self.in_dim]) * 0.01
        x = self.norm(x) * 0.01
        out = self.fc1(x)
        out = self.relu(out)
        out = self.drop(out)
...
class Predictor(nn.Module):
    def __init__(self, in_dim, fc_hidden, out_dim, dropout=0.0):
        super().__init__()
        self.in_dim = in_dim
...
class Net(nn.Module):
    def __init__(
        ...
        self.predictor = Predictor(gnn_hidden + sf_hidden, fc_hidden, out_dim, dropout)
    ...
    def forward(self, data1, data2, n_edges):
        ...
        # wandb.log(
        #     {
        #         "pre-predictor feature mean": pre_x.mean().item(),
        #         "pre-predictor feature stds": pre_x.std().item(),
        #         "post-predictor feature mean": post_x.mean().item(),
        #         "post-predictor feature std": post_x.std().item(),
        #     },
        #     commit=False,
        # )
